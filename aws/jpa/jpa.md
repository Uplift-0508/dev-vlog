...
- EntityManger 와 Entity 는 1:다 관계를 가진다. 하나의 EntityManager 인스턴스는 다수의 Entity 를 관리할 수 있다. 
  ...
  대부분의 당대 어플리케이션들은 데이터를 저장하기 위해서 관계형 데이터베이스를 사용한다. 최근에는 많은 회사들이, 데이터 유지 관리를 덜기 위해서, 객체형 데이터베이스로 바꾸고 있다. 이것은 객체형 데이터베이스나 객체 관계형 기술들이 저장, 검색, 업데이트, 유지 관리 처리하고 있는 것을 의미한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY4MDc0NTQ5OCw0MDc5NDY1MTksMTc3NT
